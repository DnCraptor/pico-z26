...
/// TODO:
		srv_colortab_hi[i]  = (red << 24) | (grn << 16) | blu; ///SDL_MapRGB(srv_screen->format, red, grn, blu);
		srv_colortab_med[i] = (red << 24) | (grn << 16) | blu; ///SDL_MapRGB(srv_screen->format, (red*med)/100, (grn*med)/100, (blu*med)/100);
		srv_colortab_low[i] = (red << 24) | (grn << 16) | blu; ///SDL_MapRGB(srv_screen->format, (red*hi)/100,  (grn*hi)/100,  (blu*hi)/100);
	}
}
...
void srv_set_screen_size(void)
{
	...
	//	SDL_GetCurrentDisplayMode(0, &DM);
		screen_width = 320; // DM.w;
		screen_height = 240; //DM.h;
	}
